  ...
  return grid.reduce(
    (count, row, x) =>
      ...
      row.reduce((rowCount, _, y) => {
        ...
        const xmas3 = +["XMAS", "SAMX"].includes(grid[x]?.[y] + grid[x + 1]?.[y + 1] + grid[x + 2]?.[y + 2] + grid[x + 3]?.[y + 3]);
        const xmas4 = +["XMAS", "SAMX"].includes(grid[x]?.[y] + grid[x + 1]?.[y - 1] + grid[x + 2]?.[y - 2] + grid[x + 3]?.[y - 3]);
